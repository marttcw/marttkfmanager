...
def menu_l_add_command(menu_l):
    ...
    menu_l.add_command(label=_("Cut"), command=lambda: target_cut())
    menu_l.add_command(label=_("Copy"), command=lambda: target_copy())
...
def about():
    ...
    about_img_render = ImageTk.PhotoImage(Image.open(project_dir+"/beta_logo.png"), master=about_win)
...
def side_file_preview(*args):       # SIDE PREVIEW DEVELOPMENT
    ...
    if tag_get == 'images' and os.path.isdir(item_media) == False and main.winfo_width() >= 800:         # Per tags go with each proper instruction
        ...
        text_render = Text(side_frame, width=80, height=int(tree['height'] * 1.5), wrap=WORD, yscrollcommand=text_scrollbar.set)
...
def tree_height_updater(old_height, old_width, old_dir, height, width, cwd, old_num):
    ...
    if 'side_frame' in globals() and (width != old_width):
        if 'img_label' in globals() and 'img_load' in globals() and img_label != None and img_load != None:
            ...
            try:
                text_render['height'] = int(tree['height'] * 1.5)
            except:
                pass
        old_width = width
